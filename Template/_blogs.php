<!--Blogs-->
<section id="blogs">
<div class="container py-4">
    <h4 class="font-rubik font-size-20">Latest Blogs</h4>
    <hr>
    <div class="owl-carousel owl-theme">
    <div class="item">
        <div class="card border-0 mr-5 font-rale" style="width: 18rem;">
        <h5 class="card-title font-size-16">Upcoming Phones</h5>
        <img src="./assets/blog/blog1.jpg" alt="card image" class="card-img-top">
        <p class="card-text font-size-16 text-black-50 py-1">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Nemo necessitatibus modi dolor nihil suscipit ipsa dolorum non et voluptatibus similique?</p>
        <a href="#" class="color-second text-left">Go somewhere</a>
        </div>
    </div>
    <div class="item">
        <div class="card border-0 mr-5 font-rale" style="width: 18rem;">
        <h5 class="card-title font-size-16">Upcoming Phones</h5>
        <img src="./assets/blog/blog2.jpg" alt="card image" class="card-img-top">
        <p class="card-text font-size-16 text-black-50 py-1">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Nemo necessitatibus modi dolor nihil suscipit ipsa dolorum non et voluptatibus similique?</p>
        <a href="#" class="color-second text-left">Go somewhere</a>
        </div>
    </div>
    <div class="item">
        <div class="card border-0 mr-5 font-rale" style="width: 18rem;">
        <h5 class="card-title font-size-16">Upcoming Phones</h5>
        <img src="./assets/blog/blog3.jpg" alt="card image" class="card-img-top">
        <p class="card-text font-size-16 text-black-50 py-1">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Nemo necessitatibus modi dolor nihil suscipit ipsa dolorum non et voluptatibus similique?</p>
        <a href="#" class="color-second text-left">Go somewhere</a>
        </div>
    </div>
    </div>
</div>
</section>
<!--!Blogs-->